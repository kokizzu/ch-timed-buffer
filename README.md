...
```
	...
	// 2. max buffer count, check record size so it won't use too much memory,
	//    eg. 100K x 200 byte row = will use around 2 x 20 MB (double buffer)
	...
	// if you want to exit immediately after all flushed and exit triggered
	// without this, might deadlock if:
	...
	// add callback to gracefully close other goroutine (eg. webserver) if needed
	tb.OnExitCallback = func() {}

	tb.Insert(...) // can be on other goroutine, it's thread safe, if insert > capacity, it would block

	tb.Close() // trigger exit manually (if needed)
